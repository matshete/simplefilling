...
function addingTotalPrices(){
	...
	$("#newSubTotal").val(addingTotalPrice);
	...
	if( totalPrice >= 200 ){
		...
		var disc = 30;
		var totalwithDisc = Number(totalPrice) - Number(disc);
		//$("#newSaleTotal").val(totalwithDisc);
		

		$("#newSaleTotal").attr("totalSale",totalwithDisc);

		$("#discTotal").val(totalwithDisc);

		$("#newDiscTotal").val(disc);
...
//$("#newPaymentMethod").change(function(){
	...
	//if(method == "cash"){
		...
		$("#newPaymentMethod").parent().parent().parent().children(".paymentMethodBoxes").html(

			 '<div class="col-xs-4"> <label id="">Enter Amount</label>'+ 
			 ...
			 '<div class="col-xs-4" id="getCashChange" style="padding-left:0px"><label id="">Change</label>'+
...
var datum = new FormData();
    datum.append("idSeller", idSeller);
